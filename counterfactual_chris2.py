...
class linear_explanation:
    ...
    def train_logistic(self,target,subset=False):
        ...
        self.coef_ = np.array([[ 2.9520612 ,  3.162272  , -0.16646102, -1.1076018 ,  0.01667761,
        -0.05662361,  0.00938269,  0.72919834,  1.8258145 , -0.69084793,
         1.0439055 ,  1.6735109 , -1.6442914 , -0.21416958, -0.03376526,
         2.026054  , -0.12796983, -0.4747369 , -0.07478762, -0.02936789,
         0.5730261 ,  0.75343406,  0.42748892,  0.11093338,  0.80563134,
        -0.17420757,  0.22403625, -0.05676472,  0.42681494]],
      dtype=np.float32)

        self.intercept_ = np.array([-5.1532764], dtype=np.float32)

    def decision_function(self, xs):
        out = []
        for x in xs:
            tot = self.intercept_[0]
            for ix in range(len(x)):
                tot += (self.coef_[0][ix]*x[ix])
            #res = 1 / (1 + np.exp(-tot))
            out.append(tot)
        return np.array(out)
    ...
    def build_structure(self):
        ...
        for i in range(self.cox.shape[0]):
            ...
            sign[0]=0
            dc=np.asarray((v,1-v))
            print(c.name, dc, d_count)
            input("....")
            cvar=self.counterfactual.addVars(2,lb=0,ub=dc,obj=c.MAD[0], name=c.name)
            cvar=np.asarray(cvar.values())
            dvar=self.counterfactual.addVars(d_count,lb=0,ub=1,obj=sign*c.MAD,
                                        vtype=gb.GRB.BINARY, name=c.name)
